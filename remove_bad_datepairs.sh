...
echo "May have to run this multiple times?"
 #datepairs=$(grep _ pairs_diff_starting_ranges.txt)
 #echo $datepairs
 grep _ pairs_diff_starting_ranges.txt > list.txt
 sed -i 's/_/-/g' list.txt
 for f in $(cat list.txt);
     do
     echo "calling f:" $f
     grep -nR $f run_files/* |awk -F":" '{print $2 , $1}' >temp.txt
     numfiles=$(wc -l temp.txt | awk '{print $1}')
     for i in $(seq 1 $numfiles)
         do
         lineno=$(sed -n "${i}p" temp.txt | awk '{print $1}')
